...
export default function AssociateInvoicePage() {
  const navigate = useNavigate();
  const location = useLocation();
  const { orderId } = useParams();
  const { fetchOrderDetails, sectionLoading, orderLoading } = useAssociateModule();
  ...
  if (!isLoaded && (sectionLoading || orderLoading)) {
    return (
      <div className="w-full bg-[#dfe3e8] px-4 py-8 sm:px-6 md:px-0">
        <div className="mx-auto max-w-5xl px-[1vw]">
          <div className="rounded-[20px] bg-white p-8 shadow-sm">
            <div className="text-sm font-semibold text-slate-500">Loading invoice details...</div>
          </div>
        </div>
      </div>
    );
  ...
  if (isLoaded && !order) {
    return (
      <div className="w-full bg-[#dfe3e8] px-4 py-8 sm:px-6 md:px-0">
        <div className="mx-auto max-w-5xl px-[1vw]">
          <div className="rounded-[20px] bg-white p-8 shadow-sm">
            <h1 className="text-2xl font-black uppercase text-slate-900">Invoice Not Found</h1>
            <p className="mt-2 text-sm text-slate-500">Could not find order information for invoice display.</p>
            <button
              type="button"
              onClick={() => navigate("/dashboard/associate-member/book-order")}
              className="mt-4 inline-flex items-center gap-2 rounded-full bg-[#2d58a5] px-5 py-2.5 text-sm font-bold text-white transition hover:bg-[#244887]"
            >
              <ArrowLeft size={16} />
              Back To Orders
            </button>
  ...
  return (
    <div className="w-full bg-[#dfe3e8] px-4 py-6 sm:px-6 md:px-0 print:bg-white print:p-0">
      <style>{`
        @media print {
          body * {
      ...
      <div className="mx-auto max-w-5xl px-[1vw]">
        <div
          ...
          className="rounded-[20px] bg-white p-6 font-sans text-[12px] leading-tight text-black shadow-md sm:p-10 print:rounded-none print:shadow-none"
        >
          <div className="mb-4 flex items-center justify-between">
            <button
              type="button"
              onClick={() => navigate(`/dashboard/associate-member/book-order/details/${orderId}`, { state: { order } })}
              className="no-print inline-flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-slate-700 transition hover:bg-slate-200 hover:text-slate-900"
            ...
            <button
              type="button"
              onClick={handlePrint}
              className="no-print inline-flex items-center gap-2 rounded-full bg-[#305CA7] px-4 py-2 text-xs font-bold text-white shadow transition hover:bg-[#244887]"
