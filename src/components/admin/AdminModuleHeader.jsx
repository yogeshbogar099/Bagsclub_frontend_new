...
export default function AdminModuleHeader({ session, headerData, onRefresh, user: propUser, onSignOut }) {
  const currentUser = session?.user || propUser;
  const userName = currentUser?.ownerName || currentUser?.businessName || currentUser?.name || "Admin";
  const mobile = currentUser?.mobileNumber || currentUser?.mobile;
  const memberId =
    headerData?.memberId ||
    (mobile ? `ADM-${String(mobile).slice(-4).padStart(4, "0")}` : "ADM-0000");
  const balance = headerData?.accountBalance || headerData?.walletBalance || currentUser?.walletBalance || "0.00";
          ...
          <div className="min-w-0 text-center box-border flex flex-col items-center justify-center px-1">
            <div className="space-y-0.5 box-border w-full">
              <p className="text-[13px] sm:text-[14px] md:text-[15px] leading-tight font-bold text-[#111] truncate box-border">
                {headerData?.systemName || "Admin Module"}
              </p>
